...
::: {.callout-important}
A `false` written under `extensions.preview-colour` is read as unset, so the option falls back to its default.
`text: false` matches its own default and takes effect; `code: false` does not, and inline code stays scanned.
:::
